"""Run the full pipeline for IEEE dataset only."""
...
def main() -> None:
    ...
    parser.add_argument("--skip-identify", action="store_true", help="Skip IEEE candidate generation (API or exports).")
    ...
    if not args.skip_refs:
        ...
        prune_stats = prune_slrs.run(source="ieee")
        print(
    ...
    if not args.skip_figures:
        figures.run(sp.overlap_out, sp.gaps_out, _REPO / "report" / "figures" / "ieee")

    print("Done. Outputs under:", (REPO_ROOT / "data" / "processed" / "ieee").relative_to(REPO_ROOT))
